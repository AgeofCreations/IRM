...
def read_one_notification(user_id, notification_id):
    ...
    q = notification_read.is_actual.add(user_read)

@shared_task
def delete_one_notification(user_id, notification_id):
    user_deleted = UserModel.objects.get(id=user_id)
    notification_deleted = NotificationModel.objects.get(id=notification_id)
    q = notification_deleted.is_actual.remove(user_deleted)
    q = notification_deleted.not_read.remove(user_deleted)
